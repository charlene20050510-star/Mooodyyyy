    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .character-mascot:hover {
            ...
            box-shadow: 0 16px 50px rgba(29, 185, 84, 0.4);
        }
        ...
        .note {
            ...
            font-size: 2rem;
            animation: float-notes 8s infinite ease-in-out;
        }
...
# 在你的 app.py 最後，在 if __name__ == "__main__": 之前添加這些路由
...
def debug_static():
    ...
    if os.path.exists(static_path):
        ...
        result += f"<p><strong>Files in static folder:</strong> {files}</p>"
        
        # 檢查 character.png
        img_path = os.path.join(static_path, 'character.png')
        if os.path.exists(img_path):
            size = os.path.getsize(img_path)
            result += f"<p><strong>character.png:</strong> EXISTS (size: {size} bytes)</p>"
        else:
            result += f"<p><strong>character.png:</strong> NOT FOUND</p>"
    else:
        result += "<p><strong>Static folder does not exist!</strong></p>"
    
    result += '<p><a href="/test-image">測試圖片顯示</a></p>'
    result += '<p><a href="/">回到首頁</a></p>'
    return result
...
def test_image():
    ...
    return '''
    <html>
    <head><title>圖片測試</title></head>
    <body>
        <h2>圖片測試</h2>
        <p>如果下面有圖片顯示，說明靜態文件服務正常：</p>
        <img src="/static/character.png" alt="Test Image" style="max-width: 200px; border: 2px solid red;">
        <p><a href="/debug/static">回到調試頁面</a></p>
        <p><a href="/">回到首頁</a></p>
    </body>
    </html>
    '''
